...
from tkinter import filedialog, Toplevel, simpledialog, messagebox, ttk
import cv2
...
# 编码方式自动选择器
class EncodingMethodSelector:
    def __init__(self, model, preprocess, device):
        self.model = model
        self.preprocess = preprocess
        self.device = device
        
    def gmm_threshold(self, similarities, pre):
        """使用高斯混合模型选择阈值"""
        ...
        p1 = weights[0] / (np.sqrt(2 * np.pi) * stds[0])
        p2 = weights[1] / (np.sqrt(2 * np.pi) * stds[1])
        peak_diff = np.abs(p1-p2)
        ...
        if not intersection_points:
            return np.mean(similarities) + np.std(similarities), peak_diff
        
        return max(intersection_points), peak_diff
    
    def select_encoding_method(self, sample_images, input_images):
        """自动选择最优的编码方法"""
        print("\nSelect encoding methods...")
        
        # 计算CLIP编码的peak_diff
        with torch.no_grad():
            clip_sample_embeddings = self.model.encode_image(sample_images).cpu().numpy()
            clip_input_embeddings = self.model.encode_image(input_images).cpu().numpy()
        
        clip_similarities = []
        for i, emb in tqdm(enumerate(clip_input_embeddings), desc="CLIP"):  # 使用enumerate获取索引
            cos_sim = 1 - np.array([spatial.distance.cosine(emb, sample_emb) 
                                for sample_emb in clip_sample_embeddings])
            
            max_sim = np.max(cos_sim)
            clip_similarities.append(max_sim)  

        clip_similarities = np.array(clip_similarities)  # 转换为NumPy数组
        _, clip_pd = self.gmm_threshold(clip_similarities, pre=0)
        

        # 计算preprocess编码的peak_diff
        pre_sample_embeddings = torch.flatten(sample_images, start_dim=1).cpu().numpy()
        pre_input_embeddings = torch.flatten(input_images, start_dim=1).cpu().numpy()
        
        pre_similarities = []
        for i, emb in tqdm(enumerate(pre_input_embeddings), desc="Processing"):
            cos_sim = 1 - np.array([spatial.distance.cosine(emb, sample_emb) 
                                   for sample_emb in pre_sample_embeddings])
            max_sim = np.max(cos_sim)
            pre_similarities.append(max_sim)  # 使用append而非extend
        pre_similarities = np.array(pre_similarities)
        _, pre_pd = self.gmm_threshold(pre_similarities, pre=1)
        
        print(f"CLIP encoding peak difference: {clip_pd:.4f}")
        print(f"Preprocess encoding peak difference: {pre_pd:.4f}")
        
        if clip_pd < pre_pd:
            print("Selected encoding method: Preprocess")
            return "preprocess"
        else:
            print("Selected encoding method: CLIP")
            return "clip"
...
class App:
    def __init__(self, root):
        ...
        self.loading_window = None  # 添加实例变量来存储加载窗口
    ...
    def display_contours(self, image_path):
        ...
        if hasattr(self, 'final_contours'):
            ...
            for i, contour in enumerate(self.final_contours):
                ...
                # 在框旁边标注序号
                # cv2.putText(self.contour_image, str(i+1), (int(x+w/2-4), int(y+h/2+4)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 1)
            ...
            self.confirm_button = tk.Button(right_frame, text="Start Classification", 
                                          command=self.start_classification_with_loading,
                                          width=20)
            self.confirm_button.pack(pady=20)
    ...
    def start_classification_with_loading(self):
        """开始分类并显示加载窗口"""
        # 禁用按钮防止重复点击
        ...
        self.loading_window.geometry("300x100+600+400")
        self.loading_window.resizable(False, False)
        # 设置窗口始终在最顶层
        self.loading_window.attributes('-topmost', True)
        # 添加加载提示
        label = tk.Label(self.loading_window, text="Classifying molecules...")
        label.pack(pady=10)
        ...
        # 在后台线程中执行耗时操作
        threading.Thread(target=self.classify_molecules, daemon=True).start()
    ...
    def classify_molecules(self):
        try:
            ...
            sample_vectors = {}
            # 构建 classified_count 字典，初始值为 1
            sample_names = [info['name'] for info in self.molecule_info if info.get('name')]
            classified_count = {name: 1 for name in sample_names}
            classified_count['other'] = 0  # 添加 'other' 类别，初始值为 0
            ...
            print(f"Marked Quantities: {len(sample_images)}")
            print(f"Uncategorized Quantities: {len(unknown_images)}")
            
            # 如果是自动选择编码方法
            if encoding_method == "auto":
                sample_tensors = torch.stack([self.preprocess(normalize_brightness(Image.fromarray(cv2.cvtColor(img, cv2.COLOR_BGR2RGB)))).to(self.device) 
                                            for img in rotated_sample_images])
                input_tensors = torch.stack([self.preprocess(normalize_brightness(Image.fromarray(cv2.cvtColor(img, cv2.COLOR_BGR2RGB)))).to(self.device) 
                                            for img in unknown_images])
                selector = EncodingMethodSelector(self.model, self.preprocess, self.device)
                encoding_method = selector.select_encoding_method(sample_tensors, input_tensors)
            ...
            for i in range(0, len(rotated_sample_images), max_parallel):
                ...
                sample_tensors = torch.stack([self.preprocess(normalize_brightness(Image.fromarray(cv2.cvtColor(img, cv2.COLOR_BGR2RGB)))).to(self.device) 
                                          for img in batch])
                if encoding_method == "raw":  # 使用原始特征
                    vectors = sample_tensors.reshape(sample_tensors.shape[0], -1).cpu().numpy().tolist()
                else:  # 使用CLIP编码
                    with torch.no_grad():
                        vectors = self.model.encode_image(sample_tensors).cpu().numpy().tolist()
            ...
            for i in range(0, len(unknown_images), max_parallel):
                ...
                if encoding_method == "raw":
                    # 使用原始特征
                    vectors = input_tensors.reshape(input_tensors.shape[0], -1).cpu().numpy().tolist()
                else:
                    # 使用CLIP编码
                    with torch.no_grad():
                        vectors = self.model.encode_image(input_tensors).cpu().numpy()
            ...
            for i, unknown_index in tqdm(enumerate(unknown_indices), desc="Classification"):
